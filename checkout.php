...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] === 'checkout') {
    ...
    if ($visitor_id) {
        try {
            $stmt = $pdo->prepare("UPDATE visitors SET status = 'Checked Out', time_out = CURRENT_TIMESTAMP WHERE id = :id AND status = 'Inside'");
...
try {
    if (!empty($search_query)) {
        ...
        $stmt = $pdo->prepare("SELECT id, name, phone_number, host_department, time_in FROM visitors WHERE status = 'Inside' AND (name LIKE :query OR phone_number LIKE :query) ORDER BY time_in DESC");
...
<div class="glass-panel p-8 rounded-2xl relative overflow-hidden mb-8">
    ...
    <form method="GET" action="checkout.php" class="flex flex-col md:flex-row gap-4 mb-3">
        <div class="flex-grow relative">
            ...
            <input type="text" name="q" value="<?php echo htmlspecialchars($search_query); ?>" class="input-glass w-full pl-12 shadow-sm focus:ring-rose-500 dark:focus:ring-rose-400 dark:border-rose-500/50" placeholder="Search Active Visitors by Name or Phone...">
        </div>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-cyan-900/30">
                    <?php if (empty($active_visitors)): ?>
                        <tr>
                            <td colspan="6" class="p-12 text-center text-slate-400 dark:text-cyan-700">No active visitors found.</td>
                        </tr>
                    <?php else: ?>
                                ...
                                <td class="p-4 text-right align-middle">
                                    <div class="flex justify-end gap-2">
                                        ...
                                        <form method="POST" action="checkout.php" class="inline">
                                            ...
                                            <button type="submit" class="px-3 py-1.5 text-xs font-bold rounded bg-rose-50 text-rose-600 border border-rose-200 hover:bg-rose-500 hover:text-white dark:bg-rose-900/30 dark:text-rose-400 dark:border-rose-500/50 dark:hover:bg-rose-500 dark:hover:text-white transition-colors shadow-sm">Checkout</button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
